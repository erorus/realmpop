		...
		if (window.XMLHttpRequest) {
			try {
				...
				if ("withCredentials" in req) {
	 				req.open('GET','http://census.realmpop.com/cloud.php?cloudrequesturl=1',true);
					req.onreadystatechange = function(evt) {
						if ((req.readyState == 4) && (req.status == 200)) 
					 		cloudfetch.fetchurl(req.responseText);
				 	}
				 	req.send(null);
					//jQuery.ajax('http://census.realmpop.com/cloud.php?cloudrequesturl=1', {cache: false, dataType: 'text', success: function(d,s,o) { cloudfetch.fetchurl(d); }});
				}
			} catch (e) {
				void(0);
...
cloudfetch.callback = function(o) {
	if (typeof this.fromurl == 'undefined') {
		...
		if (window.XMLHttpRequest) {
			var req = new XMLHttpRequest();
			if ("withCredentials" in req) {
				req.open('POST','http://census.realmpop.com/cloud.php?cloudreturndata=1'+(cloudfetch.hits++<cloudfetch.maxhits?'&cloudrequesturl=1':''),true);
